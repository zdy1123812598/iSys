...
<body style="background:#ffffff;">
			...
			<div class="span12">
				<div class="widget-box">
					...
					<div id="toolbar" class="btn-group">
						<button id="linkAdd" type="button" class="btn btn-default">
							<span class="glyphicon glyphicon-plus" aria-hidden="true">
							</span> 新增
						</button>
						<button id="linkDelete" type="button"
	...
	$(function() {
		var url = "linkListPagination.htm";
		...
		var columns = [ {
			...
			var onDblClickRow = function(item, $element) {
				self.location = "linkGet.htm?linkid="
						+ item.linkid;
			};
			
			$("#linkAdd").click(function() {
				self.location = "linkAdd.jsp";
			});
			...
			$("#linkDelete").click(function() {
				...
				for(i=0;i<currentTableData.length;i++){
					if(currentTableData[i].state==true){
						arrStr.push(currentTableData[i].linkid);
					}
				}
				if(arrStr.length>0){
					$.post("linkDelete.htm",{ids:arrStr.join(","),creatorid:$('#unid', window.parent.document).val()},function(result){
						if(result.code>0){
							initTable();
