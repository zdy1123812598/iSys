...
<body style="background:#ffffff;">
			...
			<div class="span12">
				<div class="widget-box">
					...
					<div id="toolbar" class="btn-group">
						<button id="organizationAdd" type="button" class="btn btn-default">
							<span class="glyphicon glyphicon-plus" aria-hidden="true">
							</span> 新增
						</button>
						<button id="organizationDelete" type="button"
							class="btn btn-default">
							<span class="glyphicon glyphicon-remove" aria-hidden="true">
							</span> 删除
						</button>
						<button id="organizationExportExcel" type="button"
							class="btn btn-default">
							<span class="glyphicon glyphicon-remove" aria-hidden="true">
							</span> 导出Excel
						</button>
	...
	$(function() {
		var url = "organizationListPagination.htm";
		...
		var columns = [ {
			...
			var onDblClickRow = function(item, $element) {
				self.location = "organizationGet.htm?organizationid="
						+ item.organizationid;
			};
			
			$("#organizationAdd").click(function() {
				self.location = "organizationAdd.jsp";
			});
			...
			$("#organizationDelete").click(function() {
				...
				for(i=0;i<currentTableData.length;i++){
					if(currentTableData[i].state==true){
						arrStr.push(currentTableData[i].organizationid);
					}
				}
				if(arrStr.length>0){
					$.post("organizationDelete.htm",{ids:arrStr.join(","),creatorid:$('#unid', window.parent.document).val()},function(result){
						if(result.code>0){
							initTable();
			...
			$("#organizationExportExcel").click(function() {
				window.location.href = "organizationExportExcel.htm";
			});
